...
### 2. 📚 学习记录（工具栏）
- **入口**：顶部导航 → **学习记录** → 笔记、**单词本**、学习资料（后续可扩展：编程语言等）
- **笔记**（归入学习记录，路由不变）
  - 笔记首页 (`/notes`)、编辑器 (`/notes/editor`)、查看 (`/notes/view/:id`)
  - **记录类型**：学习笔记、日常日记、随笔写写、算法（兼容旧分类：生活→日常日记，随笔→随笔写写）
  - 彩色统计卡片、搜索与筛选、标签系统、Markdown + 流程图等
- **单词本**（`/vocabulary`）
  - 单词、音标、词性、释义、例句、近义词、备注；生词/熟词标记与筛选
  - 音标自动获取与读音播放（API + TTS 回退）；词根词缀匹配与「添加新词根/词缀」；管理词根词缀库
  - 优先从 `public/content/vocabulary/` 加载；支持「同步到项目」（写入目录或下载 JSON）
- **学习资料**（`/learning-materials`）：学习条目（标题、类型、链接、说明、标签）管理
- **后续规划**：编程语言学习、与笔记联动等，详见 [docs/features/LEARNING_RECORDS.md](docs/features/LEARNING_RECORDS.md)

### 3. 📝 笔记系统（全新升级）⭐
- **笔记首页** (`/notes`)
  - 分类：学习笔记、日常日记、随笔写写、算法
  - 彩色统计卡片
  - 搜索和筛选功能
  - 美观的卡片网格
  - 标签系统
...
- **记账页面** (`/accounting`)
  ...
  - **Excel 导入/导出**：按 当天 / 当月 / 当年 导出 CSV（Excel 可打开）、从 CSV 导入
  - **打印 / 另存为 PDF**：调用浏览器打印，可另存为 PDF
  - 使用通用组件：Pagination、StatCard、EmptyState、Loading、DataExportImport
...
- **组件列表**（21个）：
  ...
  - 数据导出/导入/打印：`DataExportImport`（CSV 与 Excel 兼容，打印可另存为 PDF）
...
- **组件**：
  - `Header` - 顶部导航栏
  - `Footer` - 底部信息
  - `PageLayout` - 页面布局包装器（Header + Footer + main）⭐ 新增
  - `Sidebar`、`Navbar`、`Searchbar` - 导航相关
...
│   │   ├── CodeBlock.js      # 代码块组件
│   │   ├── FileManager/      # 文件管理器
│   │   ├── AlgorithmVisualizer/ # 算法可视化系统
│   │   ├── Visualizations/   # 通用可视化组件
│   │   ├── Showcase/         # 展示窗
│   │   ├── Layout/           # 布局组件
│   │   │   ├── Header/       # 顶部导航
│   │   │   ├── Footer/       # 底部信息
...
| 路由 | 页面组件 | 说明 |
|------|----------|------|
| `/notes` | NoteHome | 笔记首页（学习笔记/日常日记/随笔写写/算法） |
...
| 路由 | 页面组件 | 说明 |
|------|----------|------|
| `/portfolio` | Portfolio | 作品集展示 |
| `/products` | Products | 产品与服务展示 |
...
### 多语言支持（中英切换）
- **原则**：仅对**系统/组件内容**做中英切换（按钮、标签、提示、标题、空状态等界面文案）；**用户输入的内容**（笔记正文、商品名称、评论、动态内容等）不参与切换，按用户输入时的语言原样显示。
...
- **扩展**：新增页面或文案时，在 `I18nContext.js` 的 `en` 与 `zh` 中增加相同 Key，在组件中使用 `const { t } = useI18n();` 与 `t('Key')` 即可接入中英切换。
- **维护**：每次在 README 中新增或调整功能/页面描述时，请同步更新本小节的「已覆盖范围」，使中英切换的文档与实现保持一致。
...
### 中英切换（i18n）完整优化
- **全项目查漏补缺**：学习资料页（分类、空状态）、商品收藏页（Shop：筛选/排序/状态/分类/空状态/按钮）、产品页（分类标签）、项目架构（目录与节点文案）等已全部接入 `I18nContext`
- **统一体验**：切换 Header 语言后，上述页面与通用组件的界面文案随「中文 / English」切换；新增文案可按 README「多语言支持」一节扩展
